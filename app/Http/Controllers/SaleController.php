...
class SaleController extends Controller
{
    ...
    {   //dd($request);
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_sale['net_unit_price'] = $net_unit_price[$i]+$discount[$i];
    ...
    public function destroy($id)
    {
     ...
     public function findbillNumber(Request $request){
            $data = $request->all();
            //$lims_sale_data = Sale::find($data['bill_no']);
            $lims_sale_data = Sale::where('id', $data['bill_no'])->orWhere('reference_no', $data['bill_no'])->first();
            //dd($lims_sale_data);
            if(isset($lims_sale_data['reference_no']) && !empty($lims_sale_data['reference_no']) && isset($lims_sale_data['id']) && !empty($lims_sale_data['id'])){
                return redirect('sales/'.$lims_sale_data["id"].'/edit');
                //return redirect()->route('sales/edit', $lims_sale_data->id);
                exit;
            } 
            else{
                $url = url()->previous();
                return Redirect::to($url)->with('not_permitted', "No bill number found");
            }
    }
}
